...
# Images from gravatar for users
...
class User(UserMixin, db.Model):
    ...
    commenting = relationship('Comment', back_populates="commented_user")
...
class Comment(db.Model):
    ...
    commented_user=relationship("User", back_populates='commenting')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash(f"Hello {user.user_name}! You are registered and logged in!")
    ...
    if form.validate_on_submit():
        ...
        #     Check password:
            if check_password_hash(user.password, request.form.get("password")):
                the_current_user = load_user(str(user.id))
                login_user(the_current_user)
                flash("Logged in successfully!")
                return redirect(url_for("get_all_posts"))
            else:
                error = "The password you entered is not valid! Please try again!"
                flash(error)
                return render_template("login.html",form=form)
    return render_template("login.html", form=form)
...
def logout():
    logout_user()
    flash("You are Logged Out successfully! Have a nice day!")
    return redirect(url_for('get_all_posts'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash("Please Login first! Only registered users can comment!")
